...
[参考链接](http://www.itmind.net/assets/images/2017/jvm/jvm04.png)

## 内存分布

JVM内存主要分三大块：**堆内存、方法区**和**栈**。如下图中所示：

![](https://oscimg.oschina.net/oscnet/b964ec4ee41ca56e68dcec7ffebde3ee832.jpg)

内存是JVM中最大的一块由年轻代和老年代组成，而年轻代内存又被分成三部分，**Eden空间**、**From Survivor空间**、**To Survivor空间**,默认情况下年轻代按照**8:1:1**的比例来分配；

方法区存储类信息、常量、静态变量等数据，是线程共享的区域，为与Java堆区分，方法区还有一个别名Non-Heap(非堆)；

栈又分为java虚拟机栈和本地方法栈主要用于方法的执行。

在来一张图了解如何通过参数控制内存分布大小。

![](https://oscimg.oschina.net/oscnet/5ed3388d1f4b8cf29f8b87ce4f919d7553e.jpg)
...
![](https://oscimg.oschina.net/oscnet/8740e9356c2c47cc50626b6472f98e27976.jpg)
...
通过了解JVM参数控制的内存分布，当我们在分析内存溢出情况时可以更好的分析问题发生在哪里，解决问题的目的性更强。针对不同的环境，可以更加细致的分配JVM内存提高项目稳定性。
